...
function MathComponent() {
  ...
  const fetchBlock = async (index) => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/get/M/${index}`);
  ...
  const fetchTotalBlocks = async () => {
    try {
      const response = await axios.get("http://127.0.0.1:8000/total_blocks/M");
      ...
      <nav className="navbar navbar-expand-lg navbar-light bg-info">
        <div className="container-fluid">
          ...
          <button
            ...
            onClick={() => (window.location.href = "/push/M")}
          >
            Add
          </button>
        </div>
